...
export function BusinessLoginForm() {
  ...
  useEffect(() => {
    if (searchParams.get("error") === "config") {
      setError("服务端未配置 BUSINESS_SESSION_SECRET，请联系管理员");
      return;
    }
    fetch("/api/business-develop/auth/setup")
      .then((r) => r.json())
      .then((json) => {
  ...
  const submitLogin = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/business-develop/auth/login", {
      method: "POST",
    ...
    const from = searchParams.get("from") ?? "/business-develop";
    router.push(from);
    router.refresh();
  };
  ...
  const submitSetup = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/business-develop/auth/setup", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  if (setupMode || needsSetup) {
    return (
      <form onSubmit={submitSetup} className="card p-8 w-full max-w-sm space-y-4">
        <h1 className="text-xl font-bold text-center">初始化管理员账号</h1>
        <p className="text-sm text-slate-500 text-center">
          首次使用需创建账号，密码将以 bcrypt 加密存入数据库
        </p>
        <div>
        ...
        <div>
          ...
          <input
            ...
            required
            placeholder="BUSINESS_SETUP_SECRET"
          />
        </div>
  ...
  return (
    <form onSubmit={submitLogin} className="card p-8 w-full max-w-sm space-y-4">
      <h1 className="text-xl font-bold text-center">业务拓展登录</h1>
      <div>
        <label className="label">账号</label>
        <input
          className="input"
